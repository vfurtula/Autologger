...
class Run_gui(QMainWindow):
	...
	def setupUi(self):
		...
		self.combo0.addItems([str(i) for i in mylist0])
		self.combo0.setCurrentIndex(mylist0.index(self.points))
		
		##############################################
		
		waittime_lbl = QLabel("Wait time[s]*",self)
		self.combo1 = QComboBox(self)
		mylist1=[1,5,10,30,60,120,180,240,300]
		self.combo1.addItems([str(i) for i in mylist1])
		self.combo1.setCurrentIndex(mylist1.index(self.wait_time))
		...
		self.combo2.setCurrentIndex(mylist2.index(self.datahist))

		##############################################
		
		interp_lbl = QLabel("Data interp",self)
		self.combo3 = QComboBox(self)
		mylist3=["linear","linspace"]
		self.combo3.addItems(mylist3)
		self.combo3.setCurrentIndex(mylist3.index(self.interp))
	...
	def runstop(self):
		...
		if sender.text() in ["Plot data","*Plot data*"]:
			...
			if not os.path.isdir(str(self.cwdEdit.text())):
				self.config.read(''.join(["config.ini"]))
				self.cwd = self.config.get("Settings","cwd")
				if self.cwd != str(self.cwdEdit.text()):
					QMessageBox.warning( self, 'Message', ''.join(["The specified working directory ->\n",str(self.cwdEdit.text()),"\nis not anywhere on this computer and will be changed to ->\n",self.cwd]) )
					self.cwdEdit.setText(self.cwd)
			else:
	...
	def set_run(self):
		...
		self.worker_k2000m.signals.update.connect(self.update_k2000m)
		...
		self.worker_k2001m.signals.update.connect(self.update_k2001m)
		...
		self.worker_a34972a.signals.update.connect(self.update_a34972a)
	...
	def update_k2000m(self):
		...
		self.worker_k2000m.readyForUpdate()
	...
	def update_k2001m(self):
		...
		# Open the thread for updates
		self.worker_k2001m.readyForUpdate()
		...
		try:
			...
			for i in range(len(self.chnls)):
				if len(volt_a34972a[i])>self.points:
					if self.interp=="linear":
						#INTERPOLATION method
						fun = scipy.interpolate.interp1d(numpy.arange(len(volt_a34972a[i])),volt_a34972a[i])
						x_new = numpy.linspace(0,len(volt_a34972a[i])-1,self.points)
						volt_a34972a[i] = fun(x_new)
					elif self.interp=="linspace":
						#LINSPACE method
						indcs = [int(tal) for tal in numpy.linspace(0,len(volt_a34972a[i])-1,self.points)]
						volt_a34972a[i] = [volt_a34972a[i][ii] for ii in indcs]
			#print(len(volt_a34972a))
			#print(len(volt_a34972a[0]))
			############################################################
			# PLOT the data in different curves
			for i in range(len(self.chnls)):
				self.curves[i].setData(volt_a34972a[i])
				# Set the text and full date for the latest reading
				if len(volt_a34972a[i])>0:
					self.a34972a_vals[i].setText( ''.join(["sensor @", self.chnls[i][2:], " -> ",Quantity(volt_a34972a[i][-1],"V").render(prec=2),"\n\u0394time: ", str(datetime.timedelta(seconds=round(epoch[i][-1]-epoch[i][0])))," (",str(len(epoch[i])),"; ",Quantity(time.time()-time_s,"s").render(prec=2),")"]) )
		# Open the thread for updates
		self.worker_a34972a.readyForUpdate()
